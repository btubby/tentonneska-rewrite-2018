...
const eventClasses = ['a', 'b', 'c', 'd', 'e', 'a', 'b', 'c', 'd', 'e', 'a', 'b', 'c', 'd', 'e'];
class Events extends Component {
	...
	componentDidMount() {
		...
		let url = 'https://spreadsheets.google.com/feeds/list/' + sheet_id + '/od6/public/values?alt=json';
		...
		fetch(url)
			...
			.then(responseJson => {
				...
				responseJson.feed.entry.forEach(function(event, index) {
					...
					hours = hours - 12;
					parsed_event.datestring = `${days[dateObject.getDay()]}, ${dateObject.getDate()} ${
...
function formatEvent(event) {
	return {
		...
		gig_location: typeof event.gsx$venue !== 'undefined' ? '<br>' + event.gsx$venue.$t : '',
		gig_location_s: typeof event.gsx$venue !== 'undefined' ? event.gsx$venue.$t : '',
		gig_address: typeof event.gsx$address !== 'undefined' ? '<br>' + event.gsx$address.$t : '',
		gig_address_s: typeof event.gsx$address !== 'undefined' ? event.gsx$address.$t : '',
